...
class UNet(nn.Module):
    ...
    def __init__(self, num_channels_input, num_classes):
        super().__init__()
    ...
    def forward(self, x):
        ...
        enc4 = self.enc4(torch.cat([
            ...
            center, F.interpolate(dec4, center.size()[2:], mode='bilinear')], 1))
        enc3 = self.enc3(torch.cat([
            enc4, F.interpolate(dec3, enc4.size()[2:], mode='bilinear')], 1))
        enc2 = self.enc2(torch.cat([
            enc3, F.interpolate(dec2, enc3.size()[2:], mode='bilinear')], 1))
        enc1 = self.enc1(torch.cat([
            enc2, F.interpolate(dec1, enc2.size()[2:], mode='bilinear')], 1))

        return F.interpolate(self.final(enc1), x.size()[2:], mode='bilinear')
