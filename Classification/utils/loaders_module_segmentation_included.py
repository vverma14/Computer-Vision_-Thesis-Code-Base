...
class TrainingLoader():
    ...
    def __getitem__(self, idx):
        ...
        mask_for_segmenting_original = F.interpolate(torch.tensor(np.float32(predicted_mask.unsqueeze(0))),(224,224),mode='bilinear')
        # SEGMENT THE IMAGE USING MASK
        segmented_image_tensor = segment_image_white_mask(image_tensor,mask_for_segmenting_original[0])
        # the features
        ...
        return [segmented_image_tensor, meta_features, label]
...
def segment_image_black_mask(image,mask):
    mask_3_channel = torch.cat([mask, mask, mask])
    segmented_image_new = image * mask_3_channel
    return segmented_image_new

def segment_image_white_mask(image,mask):
    mask_3_channel = torch.cat([mask, mask, mask])
    segmented_image = image * mask_3_channel
    ones = torch.ones(1)
    segmented_image_new = torch.where(segmented_image == 0., ones, segmented_image)
    return segmented_image_new